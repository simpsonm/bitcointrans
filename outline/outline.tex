...
\end{align*}
for blocks $k=1,2,\dots,N$ where $\mu_{\tau}$ is a parameterized deterministic function of $\tau$. For example, a trend with seasonal variability can be obtained via $\mu_{\tau} = \alpha_0 + \alpha_1\tau + \alpha_2\sin(\omega \tau)$ where $alpha_{0:2}$ is an uknown parameter and $\omega$ is the known frequency of the seasonal effect.
